...
class QuestionMethodTests(TestCase):
    ...
    def test_was_published_recently_with_recent_question(self):
        """
        was_published_recently() should return True for question
...
def create_question(question_text, days):
    """
    Creates a question with the given 'question_text' published the give
    ...
    Choice.objects.create(choice_text='for test', question=q)
    return q

def create_a_empty_question(question_text, days):
    time = timezone.now() + datetime.timedelta(days=days)
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_detail_view_with_a_empty_question(self):
        """
        The detail view of a question with no choices should not display the
        question's text
        """
        empty_question = create_a_empty_question(question_text='empty question',
                                                 days=-5)
        response = self.client.get(reverse('polls:detail',
                                           args = (empty_question.id,)))
        self.assertEqual(response.status_code, 404)

    def test_detail_view_with_a_nonempty_question(self):
        question = create_question(question_text='nonempty question',
                                   days = -5)
        response = self.client.get(reverse('polls:detail',
                                   args=(question.id, )))
